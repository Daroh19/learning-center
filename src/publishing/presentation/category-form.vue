...
const{t } = useI18n();
const route = useRoute();   /*la ruta del objeto*/
const router = useRouter();  /*controlar la navegacion de rutas*/
...
const saveCategory = () => {
  const category = new Category(
      {id: isEdit.value ? route.params.id : null, name: form.value.name});
